...
def get_supabase_anon_key():
    ...
    if not key:
        raise RuntimeError(
            "SUPABASE_ANON_KEY is missing from Vercel."
        )
...
def get_openai_key_for_signing():
    ...
    with existing endpoint code.

    Job-token signing no longer depends on OpenAI.
    A dedicated JOB_TOKEN_SECRET is preferred.
    GEMINI_API_KEY is used as a fallback so no extra
...
def _job_token_key():
    return get_openai_key_for_signing().encode(
        "utf-8"
    )
