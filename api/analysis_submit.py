...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            instrument = validate_instrument(data.get("instrument"))
            trade_focus = validate_focus(data.get("trade_focus", "DAY TRADE"))
            ...
            if not owner:
                slot = reserve_analysis_slot(user_id)
                if slot == -1:
                    json_response(
                        self,
                        429,
            ...
            if not response_id:
                if reserved:
                    try:
                        ...
                        pass
                raise RuntimeError("The analysis service returned no job id.")
            ...
            if reserved and user:
                try:
                    release_analysis_slot(str(user["id"]))
                except Exception:
                    pass
